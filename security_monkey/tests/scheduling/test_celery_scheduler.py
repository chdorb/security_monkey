...
class CelerySchedulerTestCase(SecurityMonkeyTestCase):
    ...
    def test_celery_purge(self):
        from security_monkey.task_scheduler.tasks import purge_it
        with patch("security_monkey.task_scheduler.tasks.CELERY") as mock:
            purge_it()
            assert mock.control.purge.called

    @patch("security_monkey.task_scheduler.tasks.setup")
    @patch("security_monkey.task_scheduler.tasks.purge_it")
    ...
    def test_celery_beat(self, mock_expired_exceptions, mock_task_audit, mock_account_tech, mock_purge, mock_setup):
        from security_monkey.task_scheduler.tasks import setup_the_tasks
        from security_monkey.watchers.iam.iam_role import IAMRole
        from security_monkey.auditors.iam.iam_role import IAMRoleAuditor
    ...
    def test_audit_task(self, mock_store_exception, mock_audit_changes, mock_purge, mock_setup):
    ...
    @patch("security_monkey.task_scheduler.tasks.store_exception")
    def test_account_tech_task(self, mock_store_exception, mock_reporter, mock_purge, mock_setup):
